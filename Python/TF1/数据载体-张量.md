...
+ 键值对表示高维稀疏数据，包含位置indices、值values、稠密张量维度dense_shape3个属性。

+ 稀疏张量的创建和操作

  + 创建：使用SparseTensor类的构造方法
  + 操作
...
DX = tf.sparse_to_dense(X.indices, X.dense_shape, X.values)
S1 = tf.sparse_reduce_sum(X)
S2 = tf.sparse_reduce_sum(X, axis=0)
S3 = tf.sparse_reduce_sum(X, axis=1)
S4 = tf.sparse_reduce_sum(X, axis=[0,1])
S5 = tf.sparse_reduce_sum(X, axis=0, keep_dims=True)
S6 = tf.sparse_reduce_sum(X, axis=1, keep_dims=True)
S7 = tf.sparse_reduce_sum(X, axis=[0, 1], keep_dims=True)


with tf.Session() as sess:
    print("稠密矩阵：\n",DX.eval()), print()
    print("所有维度的元素的和：",S1.eval()), print()
    print("第0维的元素和：",S2.eval()), print()
    print("第1维的元素和：",S3.eval()), print()
    print("第0维和第1维的元素的和：",S4.eval()), print()
    print("维度不变，第0维的元素和：",S5.eval()), print()
    print("维度不变，第1维的元素和：\n",S6.eval()), print() 
    print("维度不变，第0维和第1维的元素和：",S7.eval()), print()
```
