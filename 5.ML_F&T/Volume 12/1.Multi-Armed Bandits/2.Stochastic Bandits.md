...
+ **探索优先算法和$\epsilon$-贪心算法的区别和联系**
  ...
  + 探索优先算法
  + $\epsilon$固定-贪心算法，如果$\epsilon=\frac{N}{T}$，那么和探索优先算法有什么区别？
  + $\epsilon$衰减-贪心算法
...
+ **clean事件和置信界**
  ...
  + 假设每个arm $a$对应一个reward tape
    ...
    + 当$n_t(a)=j$时对应的平均reward为$\bar{v}_j(a)$
...
+ **连续消除算法**
  ...
  + **连续消除算法**
    ...
    while(true):
    	选择所有的arm;
    	如果存在a*，对于其他所有的a，满足UCB[t][a] < LCB[T][a*];
    	break;
    2.利用
    丢弃其他a,只选择a*。
    ```

    + 算法分析—计算regret上界。(只需要关心clean事件，不需要关心bad事件？)
...
+ 对$\mu$的约束：
  ...
  + ```
    One usually proves regret bounds that hold for each µ ∈ F . Such
    regret bounds usually depend (only) on the dimension d , and allow for
    infinitely many arms. The drawback is that they are only as good as
    the worst case over F , which may be overly pessimistic (if the “bad”
    problem instances occur extremely rare), or overly optimistic (if one
    makes a strong assumption that does not quite hold in practice).
    ```
...
+ 自适应探索算法
  ...
  + UCB1算法
+ 均匀探索算法和自适应探索算法的区别：
  + 均匀探索算法只考虑的arm的平均reward，没有考虑arm被探索的次数。
  + 自适应探索算法不仅考虑arm的平均reward，还考虑了arm被探索的次数。
