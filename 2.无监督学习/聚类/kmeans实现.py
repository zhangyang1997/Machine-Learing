...
# 加载数据
data = fileload(filename = 'ST1-0-91.xlsx')
# list类型转换为array类型
data = np.array(data)
...
centroids = np.around(centroids, decimals=2)  #保留两位小数
clusterAssment = np.around(clusterAssment, decimals=2)
print("聚类中心\n", centroids)
print("每个样本的类别和样板到到聚类中心的距离\n", clusterAssment)
